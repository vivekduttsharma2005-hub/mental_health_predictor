(() => {
  ...
  let lastPayload = null; // remember the last submitted form data, for /explain

  // ---------------------------------------------------------
  // Draw tick marks on the gauge (0..10, every 2 units)
  // ---------------------------------------------------------
  function drawTicks() {
  ...
  function renderResult(score) {
    ...
    // reset explain block for the new result
    explainBlock.hidden = true;
    explainContent.hidden = true;
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      lastPayload = payload; // save for the /explain call
  ...
  explainBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!res.ok) {
        explainText.textContent = "Couldn't generate an explanation right now. Please try again.";
        explainContent.hidden = false;
        return;
      }
      ...
      (data.top_factors || []).forEach((f) => {
        ...
        const isUp = f.direction === "increased";
        li.innerHTML = `
          <span class="factor-name">${f.feature.replace(/_/g, " ")}: ${f.value}</span>
          <span class="factor-impact ${isUp ? "up" : "down"}">${isUp ? "▲" : "▼"} ${Math.abs(f.impact)}</span>
        `;
      ...
      explainText.textContent = "Couldn't reach the server for an explanation.";
      explainContent.hidden = false;
    } finally {
